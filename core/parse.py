...
from . import crypt_utils, cache_obj, list_obj, key_obj
...
def openFile(user_path: str, user_key: bytes) -> list:
    ...
    if os.path.exists(user_path):
        try:
            ...
            try:
                derv_key = crypt_utils.deriveKey(user_key)
                _checkKeyValid(raw_dict=raw_dict, user_key=derv_key)
            except Exception as e:
                raise ValueError(e)
            else:
                ...
                for password_block in enc_passwords:
                    ...
                    for element in password_block:
                        new_pass_block.append(crypt_utils.decryptOnePassword(element.encode("utf-8"), derv_key))
                    dec_passwords.append(new_pass_block)
                return dec_passwords
